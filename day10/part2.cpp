...
unsigned long findPaths(const Table &table, Point start)
{
    auto current = table.get(start.x, start.y);
    if (current == 9) {
        return 1;
    }
    unsigned long count = 0;
    for (const auto &direction : {UP, DOWN, LEFT, RIGHT}) {
        Point next = start + direction;
        if (next.x >= 0 && next.x < table.width && next.y >= 0 && next.y < table.height &&
            table.get(next.x, next.y) == current + 1) {
            count += findPaths(table, next);
        }
    }
    return count;
}
